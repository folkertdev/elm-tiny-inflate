module BitReader exposing (BitReader(..), State, Tree, andMap, andThen, chunksOf, decode, embed, error, exactly, exactlyBytes, fillWindow, getBit, loop, map, map2, map3, map4, readBits, runDecoder, shiftOffTag, shiftOntoTag, succeed)
            ...
            if numberOfBits == 0 then
                ...
                case fillWindow state of
                    ...
                    Ok d ->
                        ...
                        in
                        Ok ( val + base, { d | tag = newTag, bitcount = d.bitcount - numberOfBits } )
            ...
            if state.bitcount == 0 then
                ...
                case runDecoder 1 Decode.unsignedInt8 state of
                    Ok ( tag, newBuffer ) ->
                        ...
                        in
                        Ok ( bit, { state | tag = newTag, bitcount = 7, buffer = newBuffer } )
                ...
                in
                Ok ( bit, { state | tag = newTag, bitcount = state.bitcount - 1 } )
